...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadOrders() {
        const orders = OrderManager.getOrders(OrderManager.ORDER_STATUS.PENDING_RECEIPT);
        ...
        orderList.innerHTML = orders.map(order => {
            ...
            return '<div class="order-card" data-order-id="' + order.id + '">' +
                ...
                '<span class="order-status pending-receipt">待收货</span>' +
                '</div>' +
                ...
                '<span class="value">' + formatTime(order.shippedAt || '-') + '</span>' +
                '</div>' +
                (order.trackingNo ? '<div class="detail-row"><span class="label">物流单号</span><span class="value">' + order.trackingNo + '</span></div>' : '') +
                '</div>' +
                ...
                '<button class="btn btn-success confirm-btn" data-order-id="' + order.id + '">' +
                '<i class="fas fa-check-circle"></i>确认收货</button>' +
                '</div>' +
                '</div>' +
                '</div>';
        }).join('');
    ...
    function bindConfirmReceipt() {
        document.querySelectorAll('.confirm-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                const orderId = this.dataset.orderId;
                if (confirm('确认已收到商品吗？')) {
                    OrderManager.updateOrderStatus(orderId, OrderManager.ORDER_STATUS.COMPLETED);
                    loadOrders();
                }
            });
        });
